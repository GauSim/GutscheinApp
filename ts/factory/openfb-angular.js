...
angular.module('openfb', []).factory('OpenFB', function ($rootScope, $q, $window, $http) {
    var FB_LOGIN_URL = 'https://www.facebook.com/dialog/oauth', tokenStore = window.sessionStorage, fbAppId, oauthRedirectURL, deferredLogin, runningInCordova, loginProcessed;
    ...
    */
    function login(fbScope) {
        ...
        var windowsettings = 'location=no';
        windowsettings = 'toolbar=yes,closebuttoncaption=Back,presentationstyle=formsheet,toolbarposition=top,clearsessioncache=yes,clearcache=yes';

        if (oauthRedirectURL)
            loginWindow = window.open(FB_LOGIN_URL + '?client_id=' + fbAppId + '&redirect_uri=' + oauthRedirectURL + '&response_type=token&display=popup&scope=' + fbScope, '_blank', windowsettings);
    ...
    */
    function revokePermissions() {
        return api({ method: 'DELETE', path: '/me/permissions' }).success(function () {
    ...
    */
    function api(obj) {
        ...
        return $http({ method: method, url: 'https://graph.facebook.com' + obj.path, params: params }).error(function (data, status, headers, config) {
